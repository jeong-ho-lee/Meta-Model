    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                # train base sess
                for epoch in range(args.epochs_base):
                    tl, ta = base_train(self.model, trainloader, testloader, optimizer, scheduler,
                                         np.unique(train_set.targets), args, self.loss_curve, self.word_info, self.query_info)
                # tsl, tsa, logs = test(self.model, testloader, epoch, args, session,self.word_info)
                # if (tsa * 100) >= self.trlog['max_acc'][session]:
                #     self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                #     self.trlog['max_acc_epoch'] = epoch
                ...
                # lrc = scheduler.get_last_lr()[0]
                # result_list.append(
                #     'lr:%.4f,training_loss:%.5f,training_acc:%.5f,test_loss:%.5f,test_acc:%.5f' % (
                #         lrc, tl, ta, tsl, tsa))
                # print('This epoch takes %d seconds' % (time.time() - start_time),
                #         '\nstill need around %.2f mins to finish this session' % (
                #                 (time.time() - start_time) * (args.epochs_base - epoch) / 60))
                ...
                # result_list.append('Session {}, Test Best Epoch {},\nbest test Acc {:.4f}\n'.format(
                #     session, self.trlog['max_acc_epoch'], self.trlog['max_acc'][session], ))
                save_model_dir = os.path.join(args.save_path, 'model.pth')
                torch.save(dict(params=self.model.state_dict()), save_model_dir)
                ...
                if not args.not_data_init:
                    ...
                    self.model.module.mode = 'avg_cos'
                    tsl, tsa, logs = test(self.model, testloader, 0, args, session, self.word_info)
                    result_list.append('Base Session\nbest test Acc {:.4f}\n'.format(float('%.3f' % (tsa * 100)), ))
                    self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                    print('The new best test acc of base session={:.3f}'.format(self.trlog['max_acc'][session]))
