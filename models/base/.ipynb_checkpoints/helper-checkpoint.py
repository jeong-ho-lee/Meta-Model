...
def base_train(model, trainloader, testloader, optimizer, scheduler, class_list, args, loss_curve, word_info, query_info):
    print("[Base Train]")
    base_mode = model.module.mode
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        #proto = Support Set, query = Query Set
        proto, query = data[:k], data[k:] #Size; proto = 5, query = 20
        proto_label, query_label = train_label_onehot[:k], train_label_onehot[k:]
        model.eval()

        #Computing prototype vector (Mean of feature vectors) of each classes using proto (Support Set)
        with torch.no_grad():
            model.module.write_mem(proto, proto_label)
        
         # forward pass
        model.train()
        optimizer.zero_grad()
        query_label_indices = torch.argmax(query_label, dim=1)
        logits, cls_embed, prompt_embed, output = model(query, base=True)
        logits_ = logits[:, :args.base_class]
        acc = count_acc(logits_, query_label_indices)

        #################losss##
        loss_ce = F.cross_entropy(logits_, query_label_indices)
        loss_tri = triplet(cls_embed, prompt_embed['Vision'], query_info, query_label_indices,loss_curve)
        loss_kb = knowledge_boosting(prompt_embed['Language'], word_embed, query_info, query_label_indices,loss_curve)
        
        K = args.num_classes
        steepness = 10.
        s = 10.
        query_label_int = train_label[k:]
        excl = torch.cat([torch.cat((output[j, :y], output[j, y + 1:])).unsqueeze(0)for j, y in enumerate(query_label_int)], dim=0)
        phi = torch.sigmoid(steepness * ((excl + 1. / (K - 1)) - 0.5)) + torch.sigmoid(steepness * (-(excl + 1. / (K - 1)) - 0.5))
        numerator = s * torch.diagonal(output.transpose(0, 1)[query_label_int])
        denominator = torch.exp(numerator) + torch.sum(torch.exp(s * phi), dim=1)
        L = numerator - torch.log(denominator)
        loss = loss_ce + args.ED_hp*loss_tri + loss_kb - torch.mean(L)
        ########################
        
        lrc = scheduler.get_last_lr()[0]
        tl.add(loss.item(), len(train_label))
        ta.add(acc, len(train_label))
        
        tqdm_gen.set_description(
            'lrc={:.4f}, loss={:.4f}, acc={:.4f}'.\
                format(lrc, tl.item(), ta.item()))
...
def knowledge_boosting(lang_embed, word_embed, query_info, train_label, loss_curve):
    ...
    #* KL Feature
    loss_kd = F.kl_div(F.log_softmax(lang_embed/T,dim=1),
                       F.softmax(word_embed[train_label]/T,dim=1), reduction='batchmean')
...
def process_result(predictions, actual):
    ...
    actual_labels = torch.argmax(actual, dim=1)
