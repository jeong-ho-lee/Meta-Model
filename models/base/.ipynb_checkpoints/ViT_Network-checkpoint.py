...
class LoRALinear(nn.Module):
    def __init__(self, model, rank=4, scaling=0.1):
        super().__init__()
        self.rank = rank
        self.scaling = scaling
        self.weight = nn.Parameter(model.weight.clone())
        self.lora_A = nn.Parameter(torch.randn(rank, model.in_features) * 0.01)
        self.lora_B = nn.Parameter(torch.randn(model.out_features, rank) * 0.01)
...
def apply_lora_to_vit(vit_model, taskblock=2, rank=4, scaling=0.1):
    """
    Apply LoRA to the last `taskblock` layers of ViT.

    Args:
        vit_model: The ViT model (e.g., timm.create_model instance).
        taskblock: Number of layers to apply LoRA.
        rank: LoRA rank.
        scaling: LoRA scaling factor.
    """
    target_blocks = range(taskblock)

    for idx in target_blocks:
        block = vit_model.blocks[idx]

        # Replace qkv and proj with LoRALinear
        block.attn.qkv = LoRALinear(block.attn.qkv, rank, scaling)
        block.attn.proj = LoRALinear(block.attn.proj, rank, scaling)
        block.mlp.fc1 =LoRALinear(block.mlp.fc1, rank, scaling)
        block.mlp.fc2 =LoRALinear(block.mlp.fc2, rank, scaling)
...
class ViT_MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if args.scratch:
            ...
            self.encoder = timm.create_model("vit_base_patch16_224",pretrained=True,num_classes=args.num_classes,
                                drop_rate=0.,drop_path_rate=0.,drop_block_rate=None)
            # self.encoder = apply_lora_to_vit(self.encoder, taskblock=2, rank=1, scaling=0.5)
        ...
        self.fc = LoRALinear(self.fc, rank=1, scaling=0.5)
    ...
    def train_inc(self, dataloader, epochs, session, class_list, word_info, query_info):
        ...
        for idx,batch in enumerate(dataloader):
            ...
            for epoch in range(epochs):
                self.train()
                cls_feat, prompt_feat = self.prompt_encode(data_imgs ,prompt_feat=True, B_tuning=False)
    ...
    def write_mem(self,x,y):
        ...
        ''' 
        self.key_mem = self.encoder(x)
        # self.key_mem = self.neck(self.embedding.forward_conv(x))
        self.val_mem = y

        self.key_mem = torch.matmul(torch.transpose(self.val_mem,0,1), self.key_mem)
        return